...
class LINE:
  ...
  def __init__(self):
    ...
    self._headers = {'X-Line-Application': 'DESKTOPMAC 10.10.2-JAMESBOT-x64 MAC 4.5.0'}    
  ...
  def login(self, mail=None, passwd=None, cert=None, token=None, qr=False, callback=None, www=False):
    ...
    self._headers = {
              'X-Line-Application': 'DESKTOPMAC 10.10.2-JAMESBOT-x64 MAC 4.5.0', 
              'X-Line-Access': self.authToken, 
              'User-Agent': 'Line/10.10.2'
   }
  ...
  def downloadCOntent(self,url):
      path = '%s/JAMESBOT-%i.data' % (tempfile.gettempdir(),random.randint(0,9))
